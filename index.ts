function todoList() {
    const input: HTMLInputElement = <HTMLInputElement>(
        document.querySelector(".header__form-input")
    );
    const buttonCreate: HTMLElement = <HTMLElement>(
        document.querySelector(".header__form-button")
    );
    ...
    if (localStorage.length !== 0) {
        for (let i = 0; i < localStorage.length; i++) {
            ...
            const closeButton: Element | null =
                document.createElement("button");
            const textClose: Text = document.createTextNode("\u00D7");
            const textLi: Text = document.createTextNode(
